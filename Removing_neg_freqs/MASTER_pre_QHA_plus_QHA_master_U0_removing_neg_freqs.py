...
#Vs = np.linspace(220.79375, 230.008313, 2) 
...
# Trial:
#print ' nu(116.573346, 0.0812565643471 ,-22.3734162358896, 1692.1646665424985) ', nu(116.573346, 0.0812565643471 ,-22.3734162358896, 1692.1646665424985) 
#sys.exit()

# P(V, T) is working fine - checked with "Thermo*header.dat".
# This means that functions "P_sq(V, T, c, d, f)", "P(V)", "nu(V, c, d, f)" and "denu(V, c, d)" work ok.

# Calculating U(V) separately:
#Gs = []
#for V in Vs:
#       aux = []
#       for T in Ts:
...
#                       S_sq_sum += S_sq(V, T, c, d, f)
#                       ZPE_sq_sum += ZPE_sq(V, c, d, f)
#               aux.append( (1.0/N_k) * 0.5 * ZPE_sq_sum * (1/(4.3597482E-18)))

#       			(1.0/N_k) * ET_sq_sum) #\
#                            + (P(V) - (1.0/N_k) * P_sq_sum  * 1E-9 )* V * 1E+9 * 1E-30 * (1/(4.3597482E-18))\
#                            - T * (1.0/N_k) * ( (-KB * S_sq_sum) + ((1.0/T) * ET_sq_sum) ))
...
#print ' U(137.200672) ', U(137.200672)
#sys.exit()
...
ax.scatter(x_data_2, y_data_2, z_data_2, color='r', marker='o') # 'ro') #color='r', marker='o')
...
fig.savefig("Calcite_II_scattered_DFT_data_plus_GTP.pdf",  bbox_inches='tight', pad_inches=0.3)#, tight_layout() )#, bbox_inches=bbox)
