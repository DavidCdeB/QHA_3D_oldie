...
N_of_neg_freqs = subprocess.check_output('grep " -" Volume_for_freqs_isolation_3_true_problem.dat | wc -l', shell=True)
print type(N_of_neg_freqs)

N_of_neg_freqs = int(N_of_neg_freqs)
print type(N_of_neg_freqs)
#sys.exit()
# Deleting here those lines that contain negative freqs:
with open('Volume_for_freqs_isolation_3_true_problem.dat', 'r') as f,open('Volume_for_freqs_isolation_3_true_problem_neg_freqs_deleted.dat', 'w') as f2:
   for x in f:
       if '-' not in x:
...
f_2 = open('Volume_for_freqs_isolation_3_true_problem_neg_freqs_deleted.dat', 'r')
#next(f_2)
s = set()
...
VOLUMES_REPEATED,  FREQ, modes = np.loadtxt('Volume_for_freqs_isolation_3_true_problem_neg_freqs_deleted.dat',  skiprows = 1).T
...
#n_mode = factor
# Adjusting the n_mode by substracting negative freqs:
n_mode = factor - N_of_neg_freqs/n_volume
...
V, FREQ, mode = np.loadtxt('Volume_for_freqs_isolation_3_true_problem_neg_freqs_deleted.dat').T #,  skiprows = 1).T
